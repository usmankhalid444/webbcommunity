...
export default {
  ...
  data() {
    return {
      ...
      all_residents: false,
      all_non_residents: false,
    };
  },
    created() {
      let resUsers = this.users;
      let webResUsers =  resUsers.filter(value => value.is_webbresident == 1)
      let nonWebResUsers =  resUsers.filter(value => value.is_webbresident == 0)
      this.resident_users = webResUsers;
      this.none_resident_users = nonWebResUsers;
  },
  methods: {
    ...
    makeAnnouncement() {
      this.all_selected_users = [];
      this.selected_residents.map((user) => {
        this.all_selected_users.push(user.id);
      })

      this.selected_none_residents.map((user) => {
        this.all_selected_users.push(user.id);
      })
      this.announcement_data['all_residents'] = this.all_nonresidents;
      this.announcement_data['all_non_residents'] = this.all_non_residents;
      this.announcement_data['users'] = this.all_selected_users;
      // this.all_selected_users = JSON.stringify(this.selected_residents) + JSON.stringify(this.selected_none_residents);

      this.$inertia.post('/admin/announcements/store', this.announcement_data);
