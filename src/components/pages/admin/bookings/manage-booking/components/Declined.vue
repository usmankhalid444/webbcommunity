<template>
  <div>
    ...
    <div class="h-72 md:h-96 mt-10 overflow-auto w-full">
      ...
      >
        ...
        <tbody class="text-custom-darkblue font-semibold ">
          <tr
            v-for="(booking, i) in declined_bookings"
            :key="i"
    ...
    <div class="w-full text-custom-darkblue font-semibold mt-10">
      <p class="inline">Total {{ declined_bookings.length }}</p>
      <p class="float-right">
...
<script>
import BookingModalDeclined from "./modals/BookingModalDeclined";
...
export default {
  ...
  methods: {
    showModal(booking) {
      $vfm.show({
        component: BookingModalDeclined,
        bind: {
          bookings: booking,
        },
      });
