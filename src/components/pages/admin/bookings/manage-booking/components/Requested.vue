<template>
  <div>
    ...
    <div class="h-72 md:h-96 mt-10 overflow-auto w-full">
      ...
      >
        ...
        <tbody class="text-custom-darkblue font-semibold ">
          <tr
            v-for="(booking, i) in requested_bookings"
            :key="i"
    ...
    <div class="w-full text-custom-darkblue font-semibold mt-10">
      <p class="inline">Total {{ requested_bookings.length }}</p>
      <p class="float-right">
...
<script>
import BookingModalRequested from "./modals/BookingModalRequested";
...
export default {
  ...
  methods: {
    showModal(booking) {
      $vfm.show({
        component: BookingModalRequested,
        bind: {
          bookings: booking,
        },
      });
