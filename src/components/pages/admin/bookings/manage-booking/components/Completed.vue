<template>
  <div>
    ...
    <div class="h-72 md:h-96 mt-10 overflow-auto w-full">
      ...
      >
        ...
        <tbody class="text-custom-darkblue font-semibold ">
          <tr
            v-for="(booking, i) in completed_bookings"
            :key="i"
    ...
    <div class="w-full text-custom-darkblue font-semibold mt-10">
      <p class="inline">Total {{ completed_bookings.length }}</p>
      <p class="float-right">
...
<script>
import BookingModalCompleted from "./modals/BookingModalCompleted";
...
export default {
  ...
  methods: {
    showModal(booking) {
      $vfm.show({
        component: BookingModalCompleted,
        bind: {
          bookings: booking,
        },
      });
